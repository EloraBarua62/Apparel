...
const Products = () => {
  const productInfo = useAppSelector((state) => state.productSlice.productInfo) as Array<{ product_id: number; product_name: string; color: string[]; product_image: string[] }>;
    ...
    if (wishlistItem) {
      ...
      if (existingState) {
        ...
        localStorage.setItem(
          "wishlistState",
          JSON.stringify([wishlistIdentity])
        );
      }
  ...
  return (
    <div className={styles.manage_shop_page_setup}>
      <Header />
      <PageNavigation main={"Home"} sub={"Product List"} />
            ...
            {productInfo.map(
              (
                ...
                <div key={index} className={styles.card_design}>
                  <div
                    className={`${styles.overlay_container} slider-container`}
                  >
                    <Slider
                      {...settings}
